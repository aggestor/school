<div class="w-full h-full flex justify-center items-center">
    <div class="w-6/12  shadow bg-white h-[450px] rounded">
    <form method="POST" class="w-full h-full flex justify-center flex-col">
                <div class="md:w-10/12 w-11/12 mb-8 flex mx-auto">
                    <a class="h-8 w-8 rounded-full bg-sky-500 text-white grid place-items-center" href="javascript:history.go(-1)"><span class="fas fa-arrow-left"></span></a><h2 class="text-black text-center w-[90%] font-semibold text-lg">Nouvelle promotion.</h2>
                </div>
                ...
                <div class="md:w-10/12 w-full mx-auto my-2">
                    <div class=" mx-auto focus-within:font-semibold transition-colors duration-500  text-gray-700 focus-within:text-sky-500 focus-within:border-sky-500 border-2 border-transparent bg-slate-200 h-10 items-center flex rounded">
                        <input list="fac" name="faculty" type="text" placeholder="Faculté" class="bg-transparent transition-colors duration-500 placeholder:text-sm placeholder:text-gray-600 focus:text-sky-500 focus:outline-none ml-2 w-full" value="<?php if (isset($_POST['register_admin']) && empty($params['errors']['faculty'])) {echo $_POST['faculty'];}?>" />
                        <datalist id="fac">
                    ...
                    <?php if (isset($_POST['register_admin']) && !empty($params['errors']['faculty'])): ?>
                        <span class="-mt-2 text-red-500 text-xs"><?php echo $params['errors']['faculty']; ?></span>
                    <?php endif;?>
                </div>
                <div class="md:w-10/12 w-full mx-auto my-2">
                    <div class=" mx-auto focus-within:font-semibold transition-colors duration-500  text-gray-700 focus-within:text-sky-500 focus-within:border-sky-500 border-2 border-transparent bg-slate-200 h-10 items-center flex rounded">
                        <input list="dep" name="department" type="text" placeholder="Département" class="bg-transparent transition-colors duration-500 placeholder:text-sm placeholder:text-gray-600 focus:text-sky-500 focus:outline-none ml-2 w-full" value="<?php if (isset($_POST['register_admin']) && empty($params['errors']['department'])) {echo $_POST['department'];}?>" />
                        <datalist id="dep">
