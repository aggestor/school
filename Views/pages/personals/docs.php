...
<div class="md:w-11/12 w-full mx-auto md:p-4 bg-white">
    <div class="w-full flex p-2 rounded justify-between">
        <h1 class="text-center font-bold text-base md:text-xl lg:text-2xl text-gray-800">
            Dossiers du personel /<span class="text-sky-500"><?=$_SESSION['personal']['mat'] ?? ' ID = '.$_GET['id']?></span>
        </h1>
        <?php if(isset($_SESSION['admin'])) : ?>
            <a href="/admin/personals/docs/add/<?= $_GET['id']?>" class="p-1.5 bg-gray-100 flex justify-between rounded hover:bg-gray-200"><span class="lg:flex hidden">Ajouter un dossier </span><span class="fas fa-folder text-gray-500 lg:ml-3 mt-1"></span></a>
        <?php else :?>
            <a href="/profile/docs/add" class="p-1.5 bg-gray-100 flex justify-between rounded hover:bg-gray-200"><span class="lg:flex hidden">Ajouter un dossier </span><span class="fas fa-folder text-gray-500 lg:ml-3 mt-1"></span></a>
        <?php endif;?>
    </div>
</div>
<div class="md:w-11/12 w-full mx-auto flex mt-2 justify-between flex-col items-center py-1 mb-4 px-3 min-h-max h-auto rounded shadow bg-white">
    ...
    <div class="border rounded w-full flex-col mb-3 items-center md:flex-row text-gray-700 font-semibold p-2 flex justify-between">
        ...
        <span class="w-full mb-2 md:mb-0 md:w-1/12 flex justify-center">
            <?php if(isset($_SESSION['admin'])) : ?>
                <a class="mr-2 h-8 w-8 rounded bg-blue-500 text-white grid place-items-center" href="/admin/personals/docs/modify/<?=$doc->id?>"><span class="fas fa-pen"></span></a>
                <?php else : ?>
