...
<div class="w-full flex justify-between flex-col items-center py-1 mb-4 px-3 min-h-max h-auto rounded shadow bg-white">
    ...
    <div class="border rounded w-full mb-3 items-center text-gray-700 font-semibold p-2 flex justify-between">
        ...
        <span class="w-1/12 text-center <?= $data = $s->is_verified == 0 ? 'text-yellow-500' : 'text-green-500' ?>"><?= $data = $s->is_verified == 0 ? 'Non' : 'Oui' ?></span>
        <span class="w-1/12 text-center <?= $data = $s->is_registered == 0 ? 'text-yellow-500' : 'text-green-500' ?>"><?= $data = $s->is_registered == 0 ? 'Non' : 'Oui' ?></span>
        <span class="w-1/12 text-center <?= $data = $s->is_active == 0 ? 'text-yellow-500' : 'text-green-500' ?>"><?= $data = $s->is_active == 0 ? 'Non' : 'Oui' ?></span>
