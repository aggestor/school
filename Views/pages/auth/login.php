<section class="w-full relative flex items-center justify-center h-full bg-white">
    <main class="md:h-[480px] z-10 flex md:flex-row w-11/12 lg:w-8/12 border-r-2 border-white border-b-2 bg-white rounded-lg shadow shadow-gray-400">
          <form action="/login" method="POST" class="lg:w-5/12 w-full h-full flex justify-center flex-col">
                ...
                <div class="md:w-10/12 w-11/12 mx-auto my-2">
                    <div class=" mx-auto focus-within:font-semibold  text-gray-700 focus-within:text-sky-500 focus-within:border-sky-500 border-2 transition-colors duration-500 border-transparent bg-slate-200 h-10 items-center flex rounded">
                        <input id="identifier" name="email" type="email" placeholder="Addresse Email" class="bg-transparent transition-colors duration-500 placeholder:text-sm placeholder:text-gray-600 focus:text-sky-500 focus:outline-none ml-2 w-full" value="<?php if (isset($_POST['login']) && empty($params['errors']['email'])) {echo $_POST['email'];}?>" />
                    </div>
                ...
                <div class="md:w-10/12 w-11/12 mx-auto my-2">
                    <label for="personal"  class="flex items-center">
                        <input id="personal" name="personal"  class=" bg-slate-200 rounded-lg w-5 mr-4 h-5 accent-sky-500 text-2xl "  type="checkbox">  <span class="mb-0.5 text-sm text-gray-600">Se connecter comme personnel</span>
                    </label>
                </div>
                <div class="md:w-10/12 w-11/12 flex justify-between mx-auto my-4">
                    <a href="/" class="text-sky-500 font-semibold hover:text-sky-600 transition-colors duration-500 flex justify-center items-center w-3/12 h-10">&larr; Retour</a>
          ...
          <div class="hidden w-7/12 overflow-hidden rounded-r-lg h-full lg:block">
