...
<div class="w-full flex justify-between flex-col items-center py-1 mb-4 px-3 h-auto min-h-max rounded shadow bg-white">
    ...
    <?php foreach($params['functions'] as $fac) :?>
    <div class="border rounded w-full mb-3 items-center text-gray-600 font-semibold p-2 flex justify-between">
        <span class="w-1/12 text-center"><?= array_search($fac,$params['functions'])+1?></span>
        ...
        <span class="w-2/12 flex justify-center">
            <a class="mr-2 h-8 w-8 rounded bg-blue-500 text-white grid place-items-center" href="/admin/functions/update/<?=$fac->id?>"><span class="fas fa-pen"></span></a>
            <a class="mr-2 h-8 w-8 rounded bg-red-500 text-white grid place-items-center" href="/admin/functions/delete/<?=$fac->id?>"><span class="fas fa-trash"></span></a>
        </span>
    </div>
    <?php endforeach?>
</div>